#pragma once

#include "hyperverse/math.hpp"

#include <entt/entity/entity.hpp>
#include <eventpp/eventqueue.h>

namespace hyperverse {

enum class DomainEventType {
  ParticleFired,
  ParticleImpact,
  HomingMissileFired,
  HomingMissileIgnited,
  HomingMissileImpact,
  AsteroidDamaged,
  AsteroidFragmented,
  AsteroidConsumed,
  DroneTargetReleased,
  ContractAccepted,
  CargoEscortStarted,
  CargoArrivedAtGate,
  CargoBoxCreated,
  CargoBoxStateChanged,
  CargoDroneJobQueued,
  CargoDroneJobAssigned,
  CargoBoxPickupStarted,
  CargoBoxDeliveredToGathering,
  CargoBoxExtracted,
  CargoExtractionComplete,
  ContractRoundCompleted,
  MiningTargetCycleRequested,
  EnemyTargetCycleRequested,
  RadarTargetsCleared,
  RaiderPhaseChanged,
  RaiderTaskChanged,
  GravitySlingPhaseChanged,
  PlayerDied,
  PlayerRespawnTimeoutElapsed,
  PlayerRespawned,
  PlayerRestartRequested,
  DroneExitRequested,
  DroneSpawnRequested,
  DroneBarrelRollCompleted,
  DroneDespawned,
  SystemMenuToggleRequested,
  SystemMenuSelectionChanged,
  SystemMenuConfirmed,
  SystemMenuExitSelected,
  SystemMenuRestartSelected,
  MiningTargetLockAcquired,
  MiningTargetLockCycled,
  MiningTargetLockReleased,
  EnemyTargetLockAcquired,
  EnemyTargetLockCycled,
  EnemyTargetLockReleased,
  MiningOperationStarted,
  MiningOperationStopped,
  MiningOperationUnstable,
  MiningOperationVenting,
  MiningOperationBlowout,
  MiningOperationDepleted,
};

struct DomainEvent {
  DomainEventType type{DomainEventType::ParticleFired};
  entt::entity subject{entt::null};
  entt::entity target{entt::null};
  Vec2 position{};
  float amount{0.0F};
  int count{0};
};

using DomainEventBus = eventpp::EventQueue<DomainEventType, void(const DomainEvent&)>;

}  // namespace hyperverse
